...
function verifyReportSignature(
  vcekPublicKey: crypto.KeyObject,
  report: Report
): boolean {
  ...
  // Convert the signature from AMD's little-endian format to Node.js format
  ...
  // Node.js crypto.verify with 'ieee-p1363' DSA encoding expects R || S as big-endian,
  // each padded to curve size (48 bytes for P-384)
  ...
  // Create raw signature: R || S (IEEE P1363 format)
  const rawSignature = Buffer.alloc(96);
...
export function verifyAttestation(
  ...
  // Verify report - get the KeyObject from VCEK certificate
  const vcekPublicKey = chain.vcekPublicKey;
  const isSignatureValid = verifyReportSignature(vcekPublicKey, report);
  if (!isSignatureValid) {
